...
<template>
  ...
  >
    <div class=" h-16 w-full items-center justify-between px-10">
          ...
          <li v-for="item in cart.cart_items" :key="item.product?.id">
            <div class="flex items-center gap-3">
              <div class="relative flex h-14 w-14 items-center justify-center">
                <div
                  class="border-muted-200 dark:border-muted-700 dark:bg-muted-900 relative rounded-xl border bg-white">
                  <img
    ...
    {{ formatPrice(app.total) }}<span class="letter-t mx-2">T</span>
          ...
          <div class="mt-4 flex justify between text-xs font-bold">
            ...
            <NuxtLink :to="localPath('/payment')">
              <BaseButton :disabled="cart.cart_items?.length === 0" color="none" shape="curved"
